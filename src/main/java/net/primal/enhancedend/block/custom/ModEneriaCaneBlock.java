...
public class ModEneriaCaneBlock extends SugarCaneBlock {
    public ModEneriaCaneBlock(Settings settings) {
        super(settings);
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        if (blockState.isOf(ModBlocks.ENDIUM_SOIL) || blockState.isOf(ModBlocks.HARDENED_ENDER_MATTER)) {
            BlockPos blockPos = pos.down();
            for (Direction direction : Direction.Type.HORIZONTAL) {
                FluidState fluidState = world.getFluidState(blockPos.offset(direction));
                if (!fluidState.isOf(ModFluids.STILL_ENDER_MATTER)) continue;
                return true;
            }
        }
        return false;
