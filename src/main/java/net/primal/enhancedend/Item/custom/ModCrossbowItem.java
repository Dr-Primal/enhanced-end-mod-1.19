...
public class ModCrossbowItem
    ...
    public ModCrossbowItem(Item.Settings settings) {
        super(settings);
    }
    @Override
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (net.primal.enhancedend.Item.custom.ModCrossbowItem.isCharged(itemStack)) {
            net.primal.enhancedend.Item.custom.ModCrossbowItem.shootAll(world, user, hand, itemStack,
            net.primal.enhancedend.Item.custom.ModCrossbowItem.getSpeed(itemStack), 1.0f);
            net.primal.enhancedend.Item.custom.ModCrossbowItem.setCharged(itemStack, false);
    ...
    private static float getSpeed(ItemStack stack) {
        if (net.primal.enhancedend.Item.custom.ModCrossbowItem.hasProjectile(stack, Items.FIREWORK_ROCKET)) {
            return 1.6f;
        }
        return 3.15f;
    }
    ...
    private static boolean loadProjectiles(LivingEntity shooter, ItemStack projectile) {
        ...
        for (int k = 0; k < j; ++k) {
            ...
            if (net.primal.enhancedend.Item.custom.ModCrossbowItem.loadProjectile(shooter, projectile, itemStack, k > 0, bl)) continue;
            return false;
        }
        return true;
    }
    ...
    private static boolean loadProjectile(LivingEntity shooter, ItemStack crossbow, ItemStack projectile, boolean simulated, boolean creative) {
        ...
        net.primal.enhancedend.Item.custom.ModCrossbowItem.putProjectile(crossbow, itemStack);
        return true;
    }
    ...
    public static boolean hasProjectile(ItemStack crossbow, Item projectile) {
        return net.primal.enhancedend.Item.custom.ModCrossbowItem.getProjectiles(crossbow).stream().anyMatch(s -> s.isOf(projectile));
    }
    ...
    private static void shoot(World world, LivingEntity shooter, Hand hand, ItemStack crossbow, ItemStack projectile, float soundPitch, boolean creative, float speed, float divergence, float simulated) {
        ...
        if (bl) {
            ...
            projectileEntity = net.primal.enhancedend.Item.custom.ModCrossbowItem.createArrow(world, shooter, crossbow, projectile);
    ...
    public static void shootAll(World world, LivingEntity entity, Hand hand, ItemStack stack, float speed, float divergence) {
        List<ItemStack> list = net.primal.enhancedend.Item.custom.ModCrossbowItem.getProjectiles(stack);
        float[] fs = net.primal.enhancedend.Item.custom.ModCrossbowItem.getSoundPitches(entity.getRandom());
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (i == 0) {
                net.primal.enhancedend.Item.custom.ModCrossbowItem.shoot(world, entity, hand, stack, itemStack, fs[i], bl, speed, divergence, 0.0f);
                continue;
            }
            if (i == 1) {
                net.primal.enhancedend.Item.custom.ModCrossbowItem.shoot(world, entity, hand, stack, itemStack, fs[i], bl, speed, divergence, -10.0f);
                continue;
            }
            if (i != 2) continue;
            net.primal.enhancedend.Item.custom.ModCrossbowItem.shoot(world, entity, hand, stack, itemStack, fs[i], bl, speed, divergence, 10.0f);
        }
        net.primal.enhancedend.Item.custom.ModCrossbowItem.postShoot(world, entity, stack);
    }

    private static float[] getSoundPitches(Random random) {
        boolean bl = random.nextBoolean();
        return new float[]{1.0f,
                net.primal.enhancedend.Item.custom.ModCrossbowItem.getSoundPitch(bl, random),
                net.primal.enhancedend.Item.custom.ModCrossbowItem.getSoundPitch(!bl, random)};
    }
    ...
    private static void postShoot(World world, LivingEntity entity, ItemStack stack) {
        ...
        net.primal.enhancedend.Item.custom.ModCrossbowItem.clearProjectiles(stack);
    }
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        if (!world.isClient) {
            ...
            float f = (float)(stack.getMaxUseTime() - remainingUseTicks) / (float) net.primal.enhancedend.Item.custom.ModCrossbowItem.getPullTime(stack);
            if (f < 0.2f) {
                this.charged = false;
                this.loaded = false;
            }
    ...
    public int getMaxUseTime(ItemStack stack) {
        return net.primal.enhancedend.Item.custom.ModCrossbowItem.getPullTime(stack) + 3;
    }
    ...
    private static float getPullProgress(int useTicks, ItemStack stack) {
        float f = (float)useTicks / (float) net.primal.enhancedend.Item.custom.ModCrossbowItem.getPullTime(stack);
        if (f > 1.0f) {
            f = 1.0f;
        }
        return f;
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        List<ItemStack> list = net.primal.enhancedend.Item.custom.ModCrossbowItem.getProjectiles(stack);
        if (!net.primal.enhancedend.Item.custom.ModCrossbowItem.isCharged(stack) || list.isEmpty()) {
            return;
        }
